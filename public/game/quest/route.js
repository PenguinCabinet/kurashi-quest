...
import { mergeBring, nonPhysical } from "./bring.js";
import { visibleQuests } from "./quests.js";
import { formatJa } from "./dates.js";
...
export function buildRoute(quests, me, options = {}) {
    ...
    for (const stop of stops) {
        stop.bring = mergeBring(stop.quests.map((q) => q.procedure), me);
    }
    const all = mergeBring(todo.map((q) => q.procedure), me);
    // 終わったものを残す場合でも、回数・時間・注意は「これからやる分」だけで出す
    // （終わった手続きに「期限を過ぎています」と出しても意味がないため）
    const remaining = todo.filter((q) => !q.done);
    const remainingBring = mergeBring(remaining.map((q) => q.procedure), me);
    return {
        stops,
...
export function sortByRequires(quests) {
    ...
    while (rest.length > 0) {
        ...
        const i = rest.findIndex((q) => q.procedure.requires.every((r) => !inList.has(r) || placed.has(r)));
        if (i === -1) {
...
function byPhaseThenOrder(a, b) {
    const rank = { before: 0, moveDay: 1, within14: 2 };
...
export function placeKeyOf(p) {
    if (p.where.placeKey)
        return p.where.placeKey;
    return firstSentence(p.where.text);
}
function placeNameOf(p) {
    const key = p.where.placeKey;
    if (key && PLACE_LABELS[key])
...
function warningsFor(stops, todo, allBring) {
    ...
    for (const q of todo) {
        ...
        for (let i = 0; i < q.lock.blockedBy.length; i++) {
            ...
            const sameStop = stops.some((s) => s.quests.some((x) => x.id === q.id) && s.quests.some((x) => x.id === blockerId));
    ...
    if (unverified.length > 0) {
        out.push(`窓口の名前や持ち物のうち、まだ確認できていない項目が${unverified.length}件の手続きにあります。行く前に自治体のサイトで確認してください`);
    }
    return out;
}
