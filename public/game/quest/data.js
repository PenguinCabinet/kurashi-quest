...
export function loadProcedures(raw) {
    const result = validateProcedures(raw);
    if (result.errors.length > 0) {
        const lines = result.errors.map((e) => `  - ${e.procedureId ?? "(ファイル全体)"} / ${e.field}: ${e.message}`);
        throw new Error(`procedures.json に直すところがあります:\n${lines.join("\n")}`);
    }
    return raw;
}
/** 例外を投げずに全部の問題を返す。テストと、データを直す人向けのチェッカーで使う。 */
export function validateProcedures(raw) {
    const errors = [];
    const warnings = [];
    const unverified = [];
    const err = (procedureId, field, message) => errors.push({ procedureId, field, message });
    const warn = (procedureId, field, message) => warnings.push({ procedureId, field, message });
    if (!isObject(raw)) {
        err(null, "root", "オブジェクトではありません");
        return { errors, warnings, unverified };
    }
    if (typeof raw.dataVersion !== "string")
        warn(null, "dataVersion", "文字列で入れてください");
    if (typeof raw.targetCity !== "string")
        warn(null, "targetCity", "文字列で入れてください");
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        if (typeof p.littleKnown !== "boolean")
            err(at, "littleKnown", "true / false で入れてください");
        ...
        if (!isObject(p.where) || typeof p.where.placeKey !== "string") {
        ...
        if (!Array.isArray(p.bring)) {
            ...
            for (const b of p.bring) {
                ...
                if (itemIds.has(b.id))
                    warn(at, `bring.${b.id}`, "同じ手続きの中で持ち物 id が重複しています");
                itemIds.add(b.id);
                allItemIds.add(b.id);
                const sameLabel = idsByLabel.get(b.label) ?? new Set();
                sameLabel.add(b.id);
        ...
        if (!Array.isArray(p.steps)) {
            ...
            if (p.steps.length === 0)
                warn(at, "steps", "手順が空です。シミュレーションが作れません");
            for (let s = 0; s < p.steps.length; s++) {
                ...
                if (stuck !== undefined && stuck !== null) {
                    if (!isObject(stuck) || typeof stuck.missing !== "string" || typeof stuck.message !== "string") {
                        ...
                        err(at, `steps[${s}].stuckIf.missing`, `"${stuck.missing}" が bring にありません。持ち物として先に入れてください`);
                    }
                }
            }
        }
    ...
    for (const ref of sameAsRefs) {
        if (!allItemIds.has(ref.target)) {
            ...
            err(ref.at, `bring.${ref.id}.sameAs`, `"${ref.target}" にも sameAs が付いています。別名の別名は解決しないので、本来の id を直接指してください`);
        }
    }
    // 同じ名前なのに id が違う持ち物（攻略シートで同じものが2行に見えます）
    for (const [label, sameLabel] of idsByLabel) {
        if (sameLabel.size > 1) {
            warn(null, "bring", `「${label}」が別の id（${[...sameLabel].join(" / ")}）で入っています。同じものなら id を揃えるか sameAs でまとめてください`);
        }
    }
    ...
    for (const p of list) {
        ...
        for (const r of p.requires) {
            ...
            if (r === p.id)
                err(p.id, "requires", "自分自身を前提にしています");
        }
        const d = p.deadline;
        if (isObject(d) && d.kind === "afterProcedure") {
            if (typeof d.afterId !== "string" || !ids.has(d.afterId)) {
                ...
                warn(p.id, "requires", `期限が ${d.afterId} 起算なのに requires に入っていません。順番が守られません`);
            }
        }
    }
    for (const id of findCycle(list)) {
...
function checkChecked(value, at, field, err, unverified) {
...
function checkDeadline(p, at, err, warn) {
    const d = p.deadline;
    ...
    switch (d.kind) {
        ...
        case "afterMove":
            if (typeof d.days !== "number")
                err(at, "deadline.days", "数値で入れてください");
            break;
        case "afterProcedure":
            if (typeof d.days !== "number")
                err(at, "deadline.days", "数値で入れてください");
...
function checkCond(cond, at, field, err) {
...
function findCycle(list) {
    ...
    for (const p of list) {
        if (p && typeof p.id === "string")
            byId.set(p.id, Array.isArray(p.requires) ? p.requires : []);
    }
    const state = new Map();
    const bad = new Set();
    const walk = (id, trail) => {
        ...
        if (s === "visiting") {
            ...
            for (const x of trail.slice(from === -1 ? 0 : from))
                bad.add(x);
            return;
        }
