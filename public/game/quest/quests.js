...
import { labelOf, matchCond, unknownKeys } from "./profile.js";
...
const PHASE_RANK = { before: 0, moveDay: 1, within14: 2 };
const URGENCY_RANK = {
    overdue: 0,
    today: 1,
    soon: 2,
...
export function decideNeed(p, me) {
    if (p.skipIf) {
        const { message, ...cond } = p.skipIf;
        if (matchCond(cond, me) === "yes")
            return { status: "notNeeded", message };
    }
...
export function resolveDeadline(p, me, progress, today, byId) {
    ...
    switch (d.kind) {
        case "beforeMove": {
            if (!me.movedOn)
                return unknownDeadline(d.label, "引越す日を入れると、期限が出ます");
            return base(addDays(me.movedOn, -d.days), false);
        }
        case "aroundMove": {
            if (!me.movedOn)
                return unknownDeadline(d.label, "引越した日を入れると、期限が出ます");
...
function unknownDeadline(label, note) {
    return { label, dueOn: null, daysLeft: null, urgency: "unknown", soft: false, note };
}
function urgencyOf(daysLeft, soft) {
    if (daysLeft === null)
        return "unknown";
    if (daysLeft < 0)
        return soft ? "soon" : "overdue"; // 目安の期限で「期限切れ」とは言わない
    if (daysLeft === 0)
        return "today";
    if (daysLeft <= 3)
        return "soon";
 ...
 */
export function lockOf(p, me, progress, byId) {
    const blockedBy = [];
    const ignored = [];
    for (const id of p.requires) {
        if (progress.doneAt[id])
            continue; // 終わっている
        const prereq = byId.get(id);
        const gone = progress.dismissed.includes(id) ||
            (prereq !== undefined && decideNeed(prereq, me).status === "notNeeded");
        (gone ? ignored : blockedBy).push(id);
    }
    const nameOf = (id) => byId.get(id)?.displayName ?? id;
    return {
...
export function toQuest(p, me, progress, today, byId) {
...
export function buildQuests(data, me, progress, today) {
...
export function notNeededQuests(quests) {
    return quests.filter((q) => q.need.status === "notNeeded" || q.dismissed);
}
export function groupByPhase(quests) {
    const order = ["before", "moveDay", "within14"];
    return order
        .map((phase) => ({
        phase,
 ...
 */
export function pickNext(quests) {
...
export function statsOf(quests) {
...
function unverifiedFields(p) {
    const out = [];
    if (!p.where.verified)
        out.push("どこで");
    if (!p.sayThis.verified)
        out.push("何て言う");
    if (!p.minutes.verified)
        out.push("かかる時間");
    if (p.bring.some((b) => b.verified !== true))
        out.push("持ち物");
    if (!p.sources.some((s) => s.verified === true))
        out.push("出典");
    return out;
}
